...
let selectedHouse = null; // store selected house group
...
window.addEventListener("mousemove", (event) => {
    ...
    if (intersects.length > 0) {
        ...
        if (hoveredObject.userData.lotId) {
            ...
            tooltipText.textContent = `${lotId}`;  // set tooltip text
        ...
        tooltip.style.display = 'none';  // hide tooltip when no object is hovered
    }
});
